...
public static class VisUtils {
    ...
    /// <summary>
    /// Returns current clip info or next clip info if in transition.
    /// </summary>
    public static AnimatorClipInfo[] GetNewestClipInfo(Animator anim, int layer)
        => anim.IsInTransition(layer)
            ? anim.GetNextAnimatorClipInfo(layer)
            : anim.GetCurrentAnimatorClipInfo(layer);

    /// <returns>
    /// Normalized time (0-1 for non looping animations) of the "current animation state", or "next
    /// animation state" if in transition.
    /// </returns>
    public static float GetNewestStNrmTime(Animator anim, int layer)
        => anim.IsInTransition(layer)
            ? anim.GetNextAnimatorStateInfo(layer).normalizedTime
            : anim.GetCurrentAnimatorStateInfo(layer).normalizedTime;

    /// <summary>
    /// Returns current state info or next state info if in transition.
    /// </summary>
    public static AnimatorStateInfo GetNewestStInfo(Animator anim, int layer)
        => anim.IsInTransition(layer)
            ? anim.GetNextAnimatorStateInfo(layer)
            : anim.GetCurrentAnimatorStateInfo(layer);

    /// <summary>
    /// Gets the number of samples (frames) in the AnimationClip of the current state on a
    /// specified Animator layer. Throws exceptions if the Animator, layer, or state is invalid.<br/>
    /// NOTE: The first sample/frame index in an animation clip is 0, so last frame index is
    /// (<see cref="GetNumOfSamplesOfNewestSt"/> - 1).
    /// </summary>
    /// <param name="anim">The Animator to check.</param>
    /// <param name="layer">The layer index where the state exists.</param>
    /// <returns>The number of samples (frames) in the clip.</returns>
    public static int GetNumOfSamplesOfNewestSt(Animator anim, int layer) {
        if (anim == null)
            throw new ArgumentNullException(nameof(anim), "Animator cannot be null.");
        if (layer < 0 || layer >= anim.layerCount)
            throw new ArgumentOutOfRangeException(
                nameof(layer),
                $"Layer index {layer} is out of range. Animator has {anim.layerCount} layers."
            );
        //Get all clips currently playing on the layer
        AnimatorClipInfo[] clipInfos = GetNewestClipInfo(anim, layer);
        if (clipInfos.Length == 0 || clipInfos[0].clip == null)
            throw new InvalidOperationException($"No AnimationClip found for the current state "
                + $"on layer {layer}.");
    ...
    /// <returns>
    /// The hash of the current animator state if not in transition, or the the hash of the next
    /// animator state if in transition.
    /// </returns>
    public static int HashOfActiveAnimatorState(this Animator anim, int layer) {
        return anim.IsInTransition(layer)
            ? anim.GetNextAnimatorStateInfo(layer).shortNameHash
            : anim.GetCurrentAnimatorStateInfo(layer).shortNameHash;
    }

    /// <returns>
    /// True if currently in the specified state (if not in transition) or transitioning into
    /// the specified state (if in transition) in the Animator.
    /// </returns>
    public static bool IsActiveAnimatorState(this Animator animator, int layer, int stateShortHash) {
        if (animator.IsInTransition(layer)) {
            ...
            if (next.shortNameHash == stateShortHash)
                return true;
        }
        else {
            ...
            if (current.shortNameHash == stateShortHash)
                return true;
        }
        return false;
    }
    ...
    /// </summary>
    /// <param name="newestShortNameHash">State hash of the expected newest animator state.</param>
    public static bool TryGetNewestStInfo(
        Animator anim,
        int animLayer,
        int newestShortNameHash,
        out AnimatorStateInfo info
    ) {
        info = GetNewestStInfo(anim, animLayer);
        return info.shortNameHash == newestShortNameHash;
    }
}
